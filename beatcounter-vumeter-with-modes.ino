...
void render_double_vu(unsigned int peakToPeak, bool is_beat, bool do_fade) {
    int led = map(peakToPeak, 0, maximum, -2, num_pixels/4 - 1) - 1;
    int beat_brightness = map(peakToPeak, 0, maximum, 0, 255);
    ...
    for (int j = 0; j <= num_pixels/4; j++)
    {
      // 2 "pixels" "below" the strip, to exclude the noise floor from the VU
        if(j <= led && led >= 0) {
          // set VU color up to peak
          int color = map(j, 0, num_pixels/4, 0, 255);
          strip.setPixelColor(j, Wheel(color));
          strip.setPixelColor((num_pixels/4*2)+j, Wheel(color));
          color = map(j, 0, num_pixels/4, 255, 0);
          strip.setPixelColor((num_pixels/4*2)-j, Wheel(color));
          strip.setPixelColor((num_pixels/4*4)-j, Wheel(color));
        }
        else if(do_fade) {
          fade_pixel(j);
          fade_pixel((num_pixels/4*2)+j);
          fade_pixel((num_pixels/4*2)-j);
          fade_pixel((num_pixels/4*4)-j);
        }
    }  
}
    ...
    for (int j = 0; j <= num_pixels/4; j++)
    {
      // 2 "pixels" "below" the strip, to exclude the noise floor from the VU
        if(j <= led && led >= 0) {
          ...
          strip.setPixelColor((num_pixels/4*2)+j, Wheel2(color));
          strip.setPixelColor((num_pixels/4*2)-j, Wheel2(color));
          strip.setPixelColor((num_pixels/4*4)-j, Wheel2(color));
        }
        else if(do_fade) {
          fade_pixel(j);
          fade_pixel((num_pixels/4*2)+j);
    ...
    for (int j = 0; j <= num_pixels/4; j++)
    {
      // 2 "pixels" "below" the strip, to exclude the noise floor from the VU
        if(j <= led && led >= 0) {
          ...
          strip.setPixelColor((num_pixels/4*2)+j, Wheel3(color));
          strip.setPixelColor((num_pixels/4*2)-j, Wheel3(color));
          strip.setPixelColor((num_pixels/4*4)-j, Wheel3(color));
        }
        else if(do_fade) {
          fade_pixel(j);
          fade_pixel((num_pixels/4*2)+j);
...
void fade_pixel(int pixel) {
  ...
  int old_color = strip.getPixelColor(pixel);
